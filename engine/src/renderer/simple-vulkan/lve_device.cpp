...
void LveDevice::init(nk::Platform* platform) {
  m_platform = platform;
  createInstance();
  setupDebugMessenger();
  createSurface();
...
void LveDevice::createSurface() {
  nk::PlatformWin32* native_platform = static_cast<nk::PlatformWin32*>(m_platform);

  VkWin32SurfaceCreateInfoKHR create_info = {VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR};
  create_info.hinstance = native_platform->get_hinstance();
  create_info.hwnd = native_platform->get_hwnd();

  VkResult result = vkCreateWin32SurfaceKHR(instance, &create_info, nullptr, &surface_);

...
namespace nk {
    ...
    void Swapchain::create_swapchain(const u16 width, const u16 height) {
        ...
        if (depth_format == VK_FORMAT_UNDEFINED) {
            FatalLog("Swapchain::create_swapchain Failed to find a supported depth format!");
        }
    ...
    void Swapchain::destroy_swapchain() {
        ...
        for (auto& view : m_views) {
            vkDestroyImageView(m_device->get(), view, m_vulkan_allocator);
        }
        m_views.clear();
        m_images.clear();
    ...
    VkSurfaceFormatKHR Swapchain::choose_swap_surface_format(const Dyarr<VkSurfaceFormatKHR>& available_formats) const {
    ...
    VkPresentModeKHR Swapchain::choose_swap_present_mode(const Dyarr<VkPresentModeKHR>& available_present_modes) const {
